...
# Figure out why HPrime seems to be double the calculated curvature (could just be numeric error)
#   Figure out errorCatching functions for new Tac equations (if needed)
#   Check +- on omegaPrimeInit correction (ie scaling on soln)
#   Onset of deltaOmega errors occurs as low a kPrime=19 (checked for Vg=0.28, L=1.6e-6)
...
def eulerFreq(L=None,diameter=None,T=None,K_elec=None,F_mag=None,pos=None,stepCount=None,*args,**kwargs):
	# Ensure inputs are floats as needed
	L = float(L)
	...
	# Initialize some variables to be used during loop
#	initSoln = 0.0
#	soln = 0.0
	...
	freqStart=freqs[-1][0] # Get the last result. Always start low, so increasing will find soln
...
def quickFreq(L=None,diameter=None,T=None,stepCount=None,*args,**kwargs):
	rOut=diameter / 2
	rIn=rOut - mc.wallThickness
	mCNT=mc.rhoA * 0.735 * math.pi * diameter * L
	momentInertia=(math.pi / 4) * (rOut ** 4)
	epsilon=1e-12
	xi=np.sqrt(T / (mc.E * momentInertia))
	if xi * L < 1:
		freqStart=np.sqrt(mc.E * momentInertia / (mCNT / L)) * (22.38 / L ** 2 + 0.28 * xi ** 2)
	else:
		freqStart=np.sqrt(mc.E * momentInertia / (mCNT / L)) * (2 * math.pi / L ** 2 + math.pi * xi / L)
	threshold=epsilon * freqStart
	escapeIn=9999999
	counter = 0
	resFreq = np.zeros((stepCount,1))
	...
	for k in range(0,stepCount):
		_lambda=np.sqrt(mCNT / (L * mc.E * momentInertia)) * freqStart
